...
###########################
# Family richness per web
###########################

family_richness <- predation %>%
  group_by(web) %>%
...
per_pred <- predation %>%
  ...
  summarise(links = n()) %>%
  left_join(family_richness, by = "web")
...
ggplot(per_pred, aes(x = family_richness, y = links)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw()

ggplot(per_pred, aes(x = family_richness, y = species_richness)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw()
...
ggplot(per_pred, aes(x = web, y = links/family_richness)) +
  geom_boxplot() +
  theme_bw()
