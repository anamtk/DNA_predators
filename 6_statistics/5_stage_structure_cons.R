...
# 100% clusters function ---------------------------------------------------------------------
cons_clusters <- function(x, hclust = cluster_objs){
#process <- function(df){
  cluster <- hclust[[x]]
    
  cut_df <- as.data.frame(cutree(cluster, h=0.2)) #makes conservative clusters a DF

  cut_df_cs <- cut_df %>%
    rownames_to_column(var = "sample") %>% #make same a column again
    rename("cluster_cons" = "cutree(cluster, h = 0.2)") %>% #rename the cluster column
    ...
    filter(cluster_cons_n > 1) %>% #remove clusters of 2 or fewer
    ungroup()
...
cluster_dfs <- lapply(1:length(cluster_objs), FUN = cons_clusters)

# GLM per species -------------------------------------------------------
#DF per species for statistics
HEV_100 <- cluster_dfs[[1]]
NEO_100 <- cluster_dfs[[2]]
PHH_100 <- cluster_dfs[[3]]

pred_cols <- c("HEV" = "#EEB00C", "NEO" = "#158D8E", "PHH" = "#114C54")
# HEV ---------------------------------------------------------------------

hist(HEV_100$pred_mass_mg)

m_h <- glm(pred_mass_mg ~ cluster_cons,
               data = HEV_100,
           family = "Gamma",
           na.action= "na.fail")
dredge(m_h)
summary(m_h)
fit <- simulateResiduals(m_h, plot=T)
em <- emmeans(m_h, "cluster_cons")
pairs(em)
...
clusters_HEV <- HEV_100 %>%
  ...
  theme_bw() +
  scale_y_log10() +
  labs(x= "100% similarity clusters", y = "Predator mass (mg)") +
  theme(axis.title.y= element_text(size = 20),
        axis.title.x= element_text(size = 20),
        axis.text = element_text(size = 15))

# NEO ---------------------------------------------------------------------
hist(NEO_100$pred_mass_mg)

m_n <- lm(pred_mass_mg ~ cluster_cons,
          data = NEO_100,
          na.action = "na.fail")
dredge(m_n)
summary(m_n)
fit <- simulateResiduals(m_n, plot=T)
...
hist(PHH_100$pred_mass_mg)

m_p <- lm(pred_mass_mg ~ cluster_cons,
          data = PHH_100,
          na.action = "na.fail")
...
summary(m_p)
fit <- simulateResiduals(m_p, plot=T)
...
cons_stages <- bind_rows(cluster_dfs)
#how many clustered:
cons_stages %>%
  group_by(sample_str) %>%
  summarise(n = n())
