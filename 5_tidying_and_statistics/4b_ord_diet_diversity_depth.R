...
dna_ord <- read.csv(here("data", "outputs", "5_rarefied_taxonomic_sort", "ord_prey_DNA.csv"))
...
#Presence of DNA of each type in each predator species
dna_ord$sample <- str_sub(dna_ord$sample, start = 1, end =-2) #remove "a" at end

#create a DF that summarises the population-level incidience data for 
#each diet item in each predator species

pred_pres <- dna_ord %>%
  group_by(sample, pred_ID, Order) %>% #group each predator's diet by species
  summarise(reads = sum(reads)) %>%
  group_by(pred_ID, Order) %>%
  ...
  column_to_rownames(var = "Order") %>% #set the column to rownames for analysis
  replace(., is.na(.), 0)

pred_pres <- pred_pres[rowSums(pred_pres) != 0,] #didn't remove any
...
ggplot(indivs, aes(x = pred_ID, y = species/No..Individuals)) +
  geom_hline(yintercept = 4.47, linetype = "dashed") +
  geom_hline(yintercept = 4.21 -0.197, color = "grey") +
  geom_hline(yintercept = 4.21  +0.197, color = "grey") +
